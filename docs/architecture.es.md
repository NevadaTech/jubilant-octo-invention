...
- [Registros de Decisiones de Arquitectura](#registros-de-decisiones-de-arquitectura)
...
### ADR-1: PaginatedResult Local por Puerto

**Contexto**: Multiples modulos necesitan respuestas paginadas de la API, y la forma de la paginacion puede variar entre endpoints.

**Decision**: Cada archivo de puerto define su propia interfaz `PaginatedResult<T>` localmente en lugar de importar una compartida desde `@/shared/domain`.

**Justificacion**: Diferentes endpoints del backend pueden retornar metadatos de paginacion ligeramente distintos (por ejemplo, algunos incluyen `totalPages`, otros no; algunos envuelven datos en `{ success, message, data }`, otros en `{ data, pagination }`). Al mantener `PaginatedResult<T>` local en cada puerto, cada modulo puede adaptarse a su contrato de API especifico sin forzar un tipo unico para todos. Esto tambien evita acoplar modulos a un tipo compartido que requeriria cambios coordinados cuando un solo endpoint de la API evoluciona.

### ADR-2: Contenedor de Inyeccion de Dependencias Manual

**Contexto**: La aplicacion necesita desacoplar los hooks de presentacion de los adaptadores de infraestructura para testabilidad y flexibilidad.

**Decision**: Usar un contenedor IoC escrito a mano (`src/config/di/container.ts`) en lugar de un framework de DI como InversifyJS o tsyringe.

**Justificacion**: Los frameworks de DI para frontend agregan overhead en tiempo de ejecucion (reflect-metadata, decoradores) e incrementan el tamano del bundle. La cantidad de servicios inyectables en este proyecto (aproximadamente un adaptador de repositorio por modulo) es lo suficientemente pequena como para que un objeto TypeScript plano con cableado explicito sea suficiente. El contenedor manual es completamente type-safe sin decoradores, trivialmente mockeable en tests via `setContainer()`, y no tiene dependencias adicionales.

### ADR-3: proxy.ts en Lugar de middleware.ts (Next.js 16)

**Contexto**: Next.js 16 introdujo un cambio de ruptura que renombra el punto de entrada del edge middleware de `middleware.ts` / `middleware()` a `proxy.ts` / `proxy()`.

**Decision**: El proyecto usa `src/proxy.ts` exportando `export function proxy(request: NextRequest)` y no debe renombrarse a `middleware.ts`.

**Justificacion**: Este es un requisito obligatorio del framework en Next.js 16. El archivo maneja el enrutamiento i18n (via next-intl), verificaciones de autenticacion basadas en tokens en rutas protegidas, y redirecciones de login/dashboard. Renombrarlo a `middleware.ts` causaria que la funcion edge sea silenciosamente ignorada en tiempo de ejecucion, rompiendo las guardias de autenticacion y la deteccion de locale.

### ADR-4: Cookies HttpOnly para Almacenamiento de Tokens

**Contexto**: La aplicacion necesita almacenar tokens JWT de acceso y refresco de forma segura en el navegador.

**Decision**: Los tokens se almacenan en cookies HttpOnly, Secure, SameSite=Strict a traves de rutas BFF (Backend-for-Frontend) de la API (`src/app/api/auth/{login,refresh,logout}/route.ts`). El `TokenService` en localStorage solo almacena datos no sensibles (perfil de usuario, informacion de organizacion, timestamp de expiracion del token) -- nunca los tokens en si mismos en produccion.

**Justificacion**: Las cookies HttpOnly son inaccesibles para JavaScript, lo cual elimina el vector mas comun de robo de tokens por XSS. El flag `Secure` asegura que las cookies solo se envien sobre HTTPS. `SameSite=Strict` mitiga ataques CSRF. El patron BFF mantiene el intercambio de tokens del lado del servidor para que los tokens crudos nunca aparezcan en el codigo del lado del cliente. Este enfoque es recomendado por OWASP para SPAs que se comunican con su propio backend.
