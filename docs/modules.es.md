...
Documentacion detallada de los 14 modulos de negocio del frontend.
...
**Ruta**: `src/modules/contacts/`
**Paginas**: `/dashboard/contacts/*`

### Proposito

Gestion de clientes y proveedores como contactos. Cada contacto puede clasificarse como cliente, proveedor o ambos, y opcionalmente puede vincularse a ventas.
...
Las ventas tienen un `contactId` + `contactName` opcional (unido desde la tabla Contact). Al crear una venta, el usuario puede seleccionar un contacto existente como cliente.
...
- El campo `identification` tiene una restriccion de unicidad por organizacion (`@@unique([identification, orgId])` en Prisma).
- Navegacion: elemento del sidebar despues de Returns, usando el icono `UserRoundSearch`.
- La respuesta de la API sigue el patron wrapper `{ success, message, data, timestamp }` (igual que auth/sales).
...
**Ruta**: `src/modules/imports/`
**Paginas**: `/dashboard/imports`

### Proposito

Importacion masiva de datos desde archivos Excel (.xlsx) y CSV. Soporta cinco tipos de importacion con un flujo de dos fases (Vista previa y luego Ejecucion) que valida los datos antes de confirmar los cambios.
...
```
1. Subir archivo  ->  POST /imports/preview  ->  Resultados de validacion (errores + filas preview)
2. Revisar        ->  El usuario inspecciona errores, corrige el archivo si es necesario
3. Ejecutar       ->  POST /imports/execute  ->  Registros creados en la base de datos
```
...
- Vista previa de las primeras N filas validas

**Fase 2 -- Ejecucion**: Si el usuario esta conforme con la vista previa, confirma la ejecucion. El backend procesa todas las filas validas y crea los registros correspondientes.
...
- **Encabezados**: La primera fila debe contener encabezados de columna que coincidan con los nombres de campo esperados
- **Codificacion**: UTF-8 para archivos CSV
...
**Respuesta**: `{ success, message, data: { totalRows, validRows, errors, preview }, timestamp }`
...
- Navegacion: elemento del sidebar despues de Contacts, usando el icono `Upload`.
- La fase de vista previa no modifica la base de datos; es una ejecucion en seco (dry run).
- Los archivos grandes se procesan del lado del servidor; el frontend muestra un indicador de progreso.
...
**Ruta**: `src/modules/integrations/`
**Paginas**: `/dashboard/integrations`, `/dashboard/integrations/[id]`

### Proposito

Gestion de conexiones con plataformas e-commerce externas (VTEX como primer proveedor, extensible a MercadoLibre) para sincronizacion automatica de pedidos, productos y contactos.

### Funcionalidad Principal

- Multiples conexiones simultaneas por proveedor
- Test de conectividad y sincronizacion manual
...
- Feature gated por `integrationsEnabled` en settings de la organizacion
...
> Documentacion completa en [`docs/integrations.md`](./integrations.md).
